		...
		*/	
		...
		protected override void OnSetDefaultValue()
		{
            ...
            if (this.BusinessDate == null
                || this.BusinessDate.Year <= 2000
                )
            {
                this.BusinessDate = DateTime.Today;
            }
		}
		/// <summary>
		/// before Insert
		...
		protected override void OnDeleting() {
            ...
            {
                if (this.Status == U9.VOB.HBHCommon.U9CommonBE.DocStatus.Approved
                    || this.Status == U9.VOB.HBHCommon.U9CommonBE.DocStatus.Approving
                    )
                {
                    string msg = string.Format("单据[{0}]非开立状态，不允许删除！"
                                , this.DocNo
                                );
                    throw new UFSoft.UBF.Business.BusinessException(msg);
                }

                Feedback feed = Feedback.Finder.Find("SrcDoc=@SrcDoc"
                    , new UFSoft.UBF.PL.OqlParam(this.ID)
                    );
                if (feed != null)
                {
                    string msg = string.Format("单据[{0}]已生成下游单据[{1}]，不允许删除！"
                                , this.DocNo
                                , feed.DocNo
                                );
                    throw new UFSoft.UBF.Business.BusinessException(msg);
		...
		private static FollowService CreateDefault_Extend() {
		...
		private  static FollowService CreateDefaultComponent_Extend(){
			//TODO delete next code and add your custome code here
			throw new NotImplementedException () ;
		}
