/**************************************************************
 * Description:
 * FollowServiceBListUIModelAction.cs
 * Product: U9
 * Co.    : UFSoft Group
...
{
	public partial class FollowServiceBListUIModelAction 
	{
...
this.OnLoadData_DefaultImpl(sender,e);			
        ...
        {
            object objListType = this.CurrentPart.NameValues["HBHListType"];

            if (objListType != null
                && objListType.ToString() == "PullFollowService"
                )
            {
                filterOpath = string.Format("Status={0} and ID not in (select feed.SrcDoc from  U9::VOB::Cus::HBHTianRiSheng::Feedback feed ) "
                    , (int)DocStatusData.Approved
                    )
                    ;
            }
